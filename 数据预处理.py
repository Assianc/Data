...
#导入数据集
dataset=pd.read_csv("数据集/DataCoSupplyChainDataset.csv",header= 0,encoding= 'unicode_escape')
...
sns.heatmap(numeric_data.corr(), annot=True, linewidths=.5, fmt='.1g', cmap='coolwarm')  # 相关矩阵热图展示
plt.show()
...
amount['Sales'].sum()
print('玛丽·史密斯欺诈订单总金额',amount['Sales'].sum())
